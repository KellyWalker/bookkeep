...
- (void) awakeFromNib
{
    [[NSNotificationCenter defaultCenter] addObserverForName:BookDatabaseAvailabilityNotification
                                                      object:nil
                                                       queue:nil
                                                  usingBlock:^(NSNotification* note){
                                                      self.managedObjectContext = note.userInfo[BookDatabaseAvailabilityContext];
                                                  }];
}
...
{
    self.editButtonItem.enabled = [self.tableView numberOfRowsInSection:0] > 0;
}
...
- (void) deleteSelectedBookGroups
{
    ...
    // If deleted last, stop editing
    if (indexPathsForSelectedRows.count == [self.tableView numberOfRowsInSection:0])
    {
        self.editing = NO;
    }
